...
fs.readFile('assets/inputCardList/rare.txt', {encoding: 'utf-8'}, (err1, text) => {
  if (err1) {
    throw err1
  } else {
    const outputText:{ name: string, cardURL: string, cardID: string, cardType: number}[] = [];
    ...
    textDatas.forEach(textData => {
      for (let i =0; i < textData[2]; i++) {
        let card = {
          ...
          cardURL: textData[1],
          cardID: `${textData[1]}-${Number(i)+1}`,
          cardType: Number(textData[3])
        };
        outputText.push(card);
      }
    });
    fs.writeFileSync('assets/outputCardList/rare.json', JSON.stringify(outputText), (err2) => {
      if (err2) {
        throw err2
      } else {
        console.log("出力完了");
